...
{{-- Header Section --}}
<div class="mb-10">
    <h1 class="text-4xl font-extrabold tracking-tight font-headline text-on-surface mb-2">Selamat datang kembali, {{ $user->name }}.</h1>
    <p class="text-on-surface-variant font-body">Lacak kontribusi Anda dan jelajahi ide-ide komunitas untuk kemajuan sekolah.</p>
</div>

{{-- Summary Bento Grid --}}
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
    <x-stat-card icon="auto_awesome_motion" label="Aspirasi Diajukan" :value="$totalAspirations" badge="Total" color="primary" />
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    ...
    <a href="{{ route('aspirasi.index') }}" class="text-sm font-bold text-primary flex items-center gap-1 hover:underline">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    @foreach($trending as $index => $item)
        ...
        @if($index < 3)
            <div class="bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0px_4px_32px_rgba(0,0,0,0.04)] border-none hover:translate-y-[-4px] transition-transform duration-300 {{ $index === 1 ? 'md:mt-12' : '' }}">
                <div class="p-8 {{ $index === 2 ? 'h-full flex flex-col' : '' }}">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2 text-on-surface-variant text-xs">
                            <span class="material-symbols-outlined text-sm">{{ $index === 2 ? 'schedule' : 'visibility' }}</span>
                    ...
                    <h3 class="text-xl font-bold font-headline mb-3">
                        <a href="{{ route('aspirasi.show', $item) }}" class="hover:text-primary transition-colors">{{ $item->judul }}</a>
                    </h3>
                    <p class="text-on-surface-variant text-sm mb-6 leading-relaxed">{{ Str::limit($item->isi, 120) }}</p>

                    {{-- Vote Component --}}
                    <div class="{{ $index === 2 ? 'mt-auto ' : '' }}flex items-center justify-between"
                         x-data="{
    ...
    {{-- CTA Card --}}
    <div class="bg-gradient-to-br from-primary to-primary-container p-8 rounded-xl flex flex-col justify-center items-center text-center text-white {{ $trending->count() > 2 ? 'md:mt-12' : '' }}">
        <span class="material-symbols-outlined text-5xl mb-4" style="font-variation-settings: 'FILL' 1;">lightbulb</span>
        <h3 class="text-2xl font-extrabold font-headline mb-2 leading-tight">Punya ide brilian untuk kemajuan sekolah?</h3>
        <p class="text-white/80 text-sm mb-8 max-w-xs">Suara Anda penting. Ajukan aspirasi dan bantu wujudkan lingkungan belajar yang lebih baik.</p>
        <a href="{{ route('aspirasi.create') }}" class="bg-white text-primary px-8 py-3 rounded-full font-bold shadow-xl hover:scale-105 transition-transform active:scale-95">
            Aspirasi Baru
        </a>
    </div>
</div>
