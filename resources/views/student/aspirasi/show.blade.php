...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <section class="mt-12 space-y-6" x-data="{ replyTo: null }">
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="flex-1 space-y-3">
                            ...
                            <div class="flex justify-end">
                                <button type="submit" class="bg-primary text-white px-6 py-2 rounded-lg text-sm font-bold shadow-md shadow-primary/20 hover:brightness-110 active:scale-95 transition-all">
                                    Kirim Komentar
                                </button>
                            </div>
                        </div>
            ...
            <div class="space-y-6">
                @forelse($aspirasi->comments as $comment)
                    <div class="flex gap-4">
                        <div class="w-10 h-10 shrink-0 rounded-full bg-surface-container-highest flex items-center justify-center">
                            <span class="text-xs font-bold text-on-surface-variant">{{ strtoupper(substr($comment->user->name ?? 'A', 0, 2)) }}</span>
                        </div>
                        <div class="flex-1">
                            <div class="bg-surface-container-lowest rounded-xl rounded-tl-none p-4 shadow-sm border border-surface-container-low">
                                <div class="flex items-center justify-between mb-1">
                                    <h5 class="text-sm font-bold text-on-surface">{{ $comment->user->name ?? 'Siswa' }}</h5>
                                    <span class="text-[10px] text-on-surface-variant">{{ $comment->created_at->diffForHumans() }}</span>
                                </div>
                            ...
                            <div class="mt-2 ml-2 flex items-center gap-4 text-xs font-semibold text-on-surface-variant">
                                <button type="button" @click="replyTo = replyTo === {{ $comment->id }} ? null : {{ $comment->id }}" class="hover:text-primary transition-colors flex items-center gap-1">
                                    <span class="material-symbols-outlined text-[14px]">reply</span> Balas
                                </button>
                            </div>

                            {{-- Form Balasan (Inline) --}}
                            <div x-show="replyTo === {{ $comment->id }}" x-transition class="mt-4 flex gap-3" style="display: none;">
                                <div class="w-8 h-8 shrink-0 rounded-full bg-primary-fixed flex items-center justify-center">
                                    <span class="text-[10px] font-bold text-primary">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</span>
                                </div>
                                <div class="flex-1">
                                    <form action="{{ route('comments.store', $aspirasi) }}" method="POST">
                                        @csrf
                                        ...
                                        <div class="flex flex-col gap-2">
                                            ...
                                            <div class="flex justify-end gap-2">
                                                <button type="button" @click="replyTo = null" class="px-3 py-1.5 rounded text-xs font-bold text-on-surface-variant hover:bg-surface-container-high transition-colors">Batal</button>
                                                <button type="submit" class="bg-primary text-white px-4 py-1.5 rounded text-xs font-bold shadow-md shadow-primary/20 hover:brightness-110 transition-all">Kirim Balasan</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                                ...
                                <div class="mt-4 space-y-4 border-l-2 border-surface-container-high pl-4 ml-2">
                                    @foreach($comment->replies as $reply)
                                        <div class="flex gap-3">
                                            <div class="w-8 h-8 shrink-0 rounded-full bg-surface-container-highest flex items-center justify-center">
                                                <span class="text-[10px] font-bold text-on-surface-variant">{{ strtoupper(substr($reply->user->name ?? 'A', 0, 2)) }}</span>
                                            </div>
                                            <div class="flex-1">
                                                <div class="bg-surface-container-low rounded-lg rounded-tl-none p-3">
                                                    <div class="flex items-center justify-between mb-1">
                                                        <h5 class="text-xs font-bold text-on-surface">{{ $reply->user->name ?? 'Siswa' }}</h5>
                                                        <span class="text-[9px] text-on-surface-variant">{{ $reply->created_at->diffForHumans() }}</span>
                                                    </div>
                                                    <p class="text-xs text-on-surface-variant leading-relaxed">{{ $reply->body }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
    ...
    <aside class="lg:col-span-4 space-y-6">
        ...
        @if($aspirasi->estimasi_waktu)
            <div class="bg-surface-container-lowest border border-purple-500/30 rounded-xl p-6 shadow-sm relative overflow-hidden">
                <div class="absolute top-0 right-0 p-4 opacity-10">
                    <span class="material-symbols-outlined text-6xl text-purple-600" style="font-variation-settings: 'FILL' 1;">calendar_month</span>
                </div>
