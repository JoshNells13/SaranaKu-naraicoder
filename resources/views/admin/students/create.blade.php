@extends('layouts.app')
@section('title', 'Tambah Murid | SaranaKu')
...
<div class="flex items-center gap-2 text-on-surface-variant text-sm font-medium mb-8">
    ...
    <span class="text-primary font-semibold">Tambah Murid Baru</span>
</div>

<div class="max-w-2xl mx-auto">
    <div class="bg-surface-container-lowest rounded-2xl p-8 md:p-10 shadow-sm border border-outline-variant/30">
        <header class="mb-8">
            <h2 class="font-headline text-3xl font-extrabold text-on-surface tracking-tight mb-2">Tambah Murid Baru</h2>
            <p class="text-on-surface-variant text-sm">Masukkan informasi akun murid baru di bawah ini. Akun akan langsung aktif setelah disimpan.</p>
        </header>

        <form class="space-y-6" method="POST" action="{{ route('admin.students.store') }}">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <input class="w-full pl-12 pr-4 py-4 bg-surface-container-low border-none rounded-2xl text-on-surface placeholder:text-outline/50 focus:ring-2 focus:ring-primary/20 focus:bg-surface-container-lowest transition-all"
                           id="name" name="name" value="{{ old('name') }}" placeholder="Nama lengkap siswa" type="text" required autofocus />
                </div>
                @error('name')
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <input class="w-full pl-12 pr-4 py-4 bg-surface-container-low border-none rounded-2xl text-on-surface placeholder:text-outline/50 focus:ring-2 focus:ring-primary/20 focus:bg-surface-container-lowest transition-all"
                           id="email" name="email" value="{{ old('email') }}" placeholder="siswa@saranaku.com" type="email" required />
                </div>
                @error('email')
                    <span class="text-error text-xs font-medium px-1">{{ $message }}</span>
                @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full pl-12 pr-4 py-4 bg-surface-container-low border-none rounded-2xl text-on-surface placeholder:text-outline/50 focus:ring-2 focus:ring-primary/20 focus:bg-surface-container-lowest transition-all"
                               id="kelas" name="kelas" value="{{ old('kelas') }}" placeholder="XII IPA 1" type="text" required />
                    </div>
                    @error('kelas')
                        <span class="text-error text-xs font-medium px-1">{{ $message }}</span>
                    @enderror
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input class="w-full pl-12 pr-4 py-4 bg-surface-container-low border-none rounded-2xl text-on-surface placeholder:text-outline/50 focus:ring-2 focus:ring-primary/20 focus:bg-surface-container-lowest transition-all"
                               id="jurusan" name="jurusan" value="{{ old('jurusan') }}" placeholder="IPA / IPS / MIPA" type="text" required />
                    </div>
                    @error('jurusan')
                        <span class="text-error text-xs font-medium px-1">{{ $message }}</span>
                    @enderror
